...
`[[The vault contract#^single-source]]` links to a block, by the anchor that note carries.

## Embeds

`![[Name]]` pulls the whole note in. With a section that is not found, the embed shows nothing of the note: a notice names the address, and the note's name above it leads to the whole note.
