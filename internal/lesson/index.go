...
// the two are deliberately unrelated, so a lesson note is joined to its
...
// the slot data is machine-owned and expected to be well-formed, so a
// malformed sidecar is a build fault to
// surface, not reader-facing content to tolerate. Two files claiming the same slug is likewise an error —
