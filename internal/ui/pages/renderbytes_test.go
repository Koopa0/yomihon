...
func TestRenderedBytesAreUnchanged(t *testing.T) {
	...
	cases := []surface{
		...
		// into a diff nobody can read back.
...
// englishChrome is the same page furniture in the other language, so a surface
// can be recorded twice and the pair read as one diff. The notice every empty
// and faulted surface shares is one component for both languages, and a
// sentence that only fits in one of them is a layout fault the Chinese
// recording alone cannot show.
