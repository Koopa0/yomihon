...
func TestPrintGivesThePaperItsMarginsAndKeepsBlocksWhole(t *testing.T) {
	...
	for _, want := range []struct {
		...
		{".y-findings tr", "break-inside", "avoid", "a finding and the file it is about belong on one sheet"},
		{".y-findingsguide::details-content", "content-visibility", "visible", "a printed table of faults whose words nobody can look up is a list of complaints, and the person holding the paper cannot click the fold open"},
