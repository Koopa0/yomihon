...
func SchemaSentence(lang Lang, ruleID, field, target, folder string) []SchemaPart {
	...
	case "schema.frontmatter":
		return []SchemaPart{text(schemaFrontmatterUnreadable)}
...
var (
	...
	schemaFrontmatterUnreadable = both(
		"這篇的 frontmatter 讀不出來,所以它宣告的每一項都無法判斷。",
		"This note's frontmatter could not be read, so nothing it declares could be judged.")
