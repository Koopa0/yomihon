...
// the page's title, assigning heading slugs one level down so they sit under the
// chrome title and collecting the table of contents over the assembled HTML so
// ids stay unique, and resolving assets. relPath is required because markdown
// writes an image path relative to its own note.
