...
// awaitsHumanContract loads a contract whose lifecycle covers every answer
// AwaitsHuman can give: an onward stage owned only by agents, one owned by a
// declared human, one that is the published stage, and the universal
// retirement stage. header carries the top-level human_owners line, or
// nothing to leave the declaration out.
func awaitsHumanContract(t *testing.T, header string) *schema.Contract {
	t.Helper()
...
[enums.status]
note = ["inbox", "draft", "ready", "published", "archived"]
...
// TestAwaitsHuman pins the waiting derivation the home panel counts with:
// a status waits exactly when one of its onward stages is owned by a
// declared human, the published stage never counts, the universal
// retirement stage never counts, and without the declaration nothing waits.
func TestAwaitsHuman(t *testing.T) {
	t.Parallel()

	declared := awaitsHumanContract(t, `human_owners = ["reviewer"]`+"\n")
	undeclared := awaitsHumanContract(t, "")
	...
	tests := []struct {
		...
		{"onward owned only by agents", declared, "inbox", false},
		{"onward owned by a declared human", declared, "draft", true},
		{"only onward is published", declared, "ready", false},
		{"only onward is universal retirement", declared, "published", false},
		{"no human declaration", undeclared, "draft", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			if got := tt.contract.AwaitsHuman("doc", tt.status); got != tt.want {
				t.Errorf("AwaitsHuman(%q, %q) = %v, want %v", "doc", tt.status, got, tt.want)
			}
		})
	}
}

// TestHumanOwnersRejectsBlankAndDuplicate holds the new top-level key to the
// same lexical bar every other declared list meets.
func TestHumanOwnersRejectsBlankAndDuplicate(t *testing.T) {
	t.Parallel()
	for _, tt := range []struct {
		...
		{"blank entry", `human_owners = [""]` + "\n", "human_owners: empty value"},
		{"duplicate entry", `human_owners = ["reviewer", "reviewer"]` + "\n", `human_owners: duplicate value "reviewer"`},
	} {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			text := `schema_version = "1"
...
`
			...
			if err == nil || !strings.Contains(err.Error(), tt.want) {
				t.Fatalf("LoadFile() = (%v, %v), want an error containing %q", contract, err, tt.want)
			}
		})
	}
}
