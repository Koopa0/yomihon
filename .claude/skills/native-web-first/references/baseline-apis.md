...
across the core browsers (Chrome, Edge, Firefox, Safari): *Newly available*
when all four ship it, *Widely available* ~30 months later.
...
being on `developer.chrome.com` or `chromestatus.com` tells you *Chrome* ships
...
The operator target here is *latest Chrome/Safari on one machine*, so you may
...
[data-theme="dark"]         { --bg: …; --fg: …; }
[data-ruby="off"] rt:not([data-keep="1"]) { visibility: hidden; } /* zero reflow */
[data-nav="open"] .rail { transform: none; }
```
...
class-toggling JS.
```css
.field:has(:user-invalid) .hint { color: var(--error); }
...
@container (min-width: 30rem) { .panel .row { grid-template-columns: 1fr 1fr; } }
```
Baseline 2023. Prefer when a component must adapt to *its* space, not the
viewport.
...
lagging at time of writing) avoids orphans in prose. Both are *pure niceties*
...
```js
btn.addEventListener('pointerup', () => { /* after the hold gesture */ form.requestSubmit(); });
```
...
*Newly* (Chrome/Safari solid, Firefox catching up). Treat as pure enhancement —
