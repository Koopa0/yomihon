...
**JS** flips the attribute _and_ writes the cookie so the next load matches:
...
430 ms press-and-hold as a misclick guard, then submits the _same_ form.
...
<form method="post" action="/status" data-seal>
   ...
   <button type="submit" data-seal-btn aria-label="hold to certify ready">
      Certify ready
   </button>
</form>
```
